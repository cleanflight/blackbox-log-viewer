...
function getReleaseFilename(platform, ext) {
    return 'cleanflight-blackbox-explorer_' + pkg.version + '_' + platform + '.' + ext;
}
...
function release_zip(arch) {
    ...
    return compressFiles(src, base, output, 'Cleanflight Blackbox Explorer');
}
...
function release_deb(arch, done) {
    ...
    return gulp.src([path.join(APPS_DIR, pkg.name, arch, '*')])
        .pipe(deb({
             ...
             postinst: ['xdg-desktop-menu install /opt/cleanflight/cleanflight-blackbox-explorer/cleanflight-blackbox-explorer.desktop'],
             prerm: ['xdg-desktop-menu uninstall cleanflight-blackbox-explorer.desktop'],
             depends: 'libgconf-2-4',
             changelog: [],
             _target: 'opt/cleanflight/cleanflight-blackbox-explorer',
             _out: RELEASE_DIR,
...
function release_osx64() {
    ...
    return gulp.src(['.'])
        .pipe(appdmg({
            ...
            specification: {
                title: 'CF Blackbox Explorer', // <= volume name; should be smaller than 27 chars.
                contents: [
